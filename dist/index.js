...
import { DEFAULT_CODEX_AUTH_PATH, DEFAULT_MODEL as CODEX_DEFAULT_MODEL, readCodexCredentials, } from "./src/codex-auth.js";
...
function saveGrokExpiry(info) {
    saveProviderExpiry("grok", info);
}
function loadGrokExpiry() {
...
async function scanCookieExpiry(ctx) {
    try {
...
let _cdpBrowserLaunchPromise = null;
 ...
 */
async function connectToOpenClawBrowser(log) {
 ...
 */
async function getOrLaunchGrokContext(log) {
    ...
    _cdpBrowserLaunchPromise = (async () => {
        ...
        try {
            ...
            log(`[cli-bridge:grok] failed to launch browser: ${err.message}`);
            return null;
        }
        finally {
...
let _geminiLaunchPromise = null;
async function getOrLaunchGeminiContext(log) {
    if (geminiContext) {
        try {
            geminiContext.pages();
    ...
    _geminiLaunchPromise = (async () => {
        ...
        try {
            ...
            log(`[cli-bridge:gemini] failed to launch browser: ${err.message}`);
            return null;
        }
        finally {
...
let _claudeLaunchPromise = null;
async function getOrLaunchClaudeContext(log) {
    if (claudeWebContext) {
        try {
            claudeWebContext.pages();
    ...
    _claudeLaunchPromise = (async () => {
        ...
        try {
            ...
            log(`[cli-bridge:claude-web] failed to launch browser: ${err.message}`);
            return null;
        }
        finally {
...
let _chatgptLaunchPromise = null;
async function getOrLaunchChatGPTContext(log) {
    if (chatgptContext) {
        try {
            chatgptContext.pages();
    ...
    _chatgptLaunchPromise = (async () => {
        ...
        try {
            ...
            log(`[cli-bridge:chatgpt] failed to launch browser: ${err.message}`);
            return null;
        }
        finally {
...
async function sessionKeepAlive(log) {
    const providers = [
    ...
    for (const p of providers) {
        ...
        try {
            ...
            log(`[cli-bridge:${p.name}] session keep-alive failed: ${err.message}`);
        }
...
async function cleanupBrowsers(log) {
 ...
 */
async function ensureAllProviderContexts(log) {
...
async function _doEnsureAllProviderContexts(log) {
    const { chromium } = await import("playwright");
    ...
    try {
        ...
        if (sharedCtx)
            log("[cli-bridge] using OpenClaw browser for all providers");
    }
    catch { /* not available */ }
    ...
    for (const cfg of providerConfigs) {
        ...
        if (!ctx) {
            try {
                ...
                log(`[cli-bridge:${cfg.name}] could not launch browser: ${err.message}`);
            }
        }
        if (ctx)
            cfg.setCtx(ctx);
...
async function tryRestoreGrokSession(_sessionPath, log) {
    try {
        ...
        if (expiry) {
            ...
            if (freshExpiry)
                saveGrokExpiry(freshExpiry);
        }
        return true;
    }
    catch (err) {
        log(`[cli-bridge:grok] session restore error: ${err.message}`);
        return false;
    }
}
...
// ──────────────────────────────────────────────────────────────────────────────
async function checkBitNetServer(url = "http://127.0.0.1:8082") {
    return new Promise((resolve) => {
        const target = new URL("/v1/models", url);
        const req = http.get({ hostname: target.hostname, port: parseInt(target.port), path: target.pathname, timeout: 3_000 }, (res) => {
            let data = "";
...
// ──────────────────────────────────────────────────────────────────────────────
async function applyModelSwitch(api, model, label) {
    ...
    try {
        const result = await api.runtime.system.runCommandWithTimeout(["openclaw", "models", "set", model], { timeoutMs: 8_000 });
        if (result.code !== 0) {
...
// ──────────────────────────────────────────────────────────────────────────────
async function switchModel(api, model, label, forceNow) {
    ...
    return {
        text: `📋 **Model switch staged: ${label}**\n` +
            `\`${model}\`\n\n` +
...
// ──────────────────────────────────────────────────────────────────────────────
function proxyTestRequest(port, apiKey, model, timeoutMs) {
...
const plugin = {
    ...
    description: "Phase 1: openai-codex auth bridge. " +
    ...
    register(api) {
        ...
        if (!_startupRestoreDone && _proxyOwnedByThisProcess) {
            ...
            void (async () => {
                ...
                for (const p of profileProviders) {
                    ...
                    if (cookiesValid) {
                        try {
                            ...
                            p.setCtx(ctx);
                            ctx.on("close", () => { p.setCtx(null); });
                            api.logger.info(`[cli-bridge:${p.name}] session restored from profile ✅`);
                        }
                        catch (err) {
                            api.logger.warn(`[cli-bridge:${p.name}] context launch failed: ${err.message}`);
                            needsLogin.push(p.loginCmd);
                        }
                    ...
                    try {
                        ...
                        api.logger.warn(`[cli-bridge:${p.name}] startup restore failed: ${err.message}`);
                    }
                    // Sequential — never spawn all 4 Chromium instances at once
                    await new Promise(r => setTimeout(r, 3000));
                }
                ...
                if (needsLogin.length > 0) {
                    ...
                    try {
                        await api.runtime.system.runCommandWithTimeout(["openclaw", "message", "send", "--channel", "discord", "--to", "1480601811537760487", "--message", msg], { timeoutMs: 10_000 });
                        api.logger.info(`[cli-bridge] sent re-login notification for: ${needsLogin.join(", ")}`);
                    }
                    catch (err) {
                        api.logger.warn(`[cli-bridge] failed to send re-login notification: ${err.message}`);
                    }
                }
            })();
                    ...
                    void (async () => {
                        ...
                        if (failed.length > 0) {
                            ...
                            try {
                                ...
                                api.logger.warn(`[cli-bridge] keep-alive: failed to send notification: ${err.message}`);
                            }
                        }
                    })();
                }, 72_000_000);
        ...
        if (enableProxy) {
            ...
            const probeExisting = () => {
                return new Promise((resolve) => {
                    const req = http.request({ hostname: "127.0.0.1", port, path: "/v1/models", method: "GET",
                        headers: { Authorization: `Bearer ${apiKey}` } }, (res) => { res.resume(); resolve(res.statusCode === 200); });
            ...
            const startProxy = async () => {
                ...
                try {
                    ...
                    const _agentDefaults = api.pluginConfig?.["_resolvedAgentDefaults"];
                    const _llmConfig = _agentDefaults?.["llm"];
                    const llmIdleTimeout = _llmConfig?.["idleTimeoutSeconds"];
                    ...
                    if (msg.includes("EADDRINUSE")) {
                        ...
                        try {
                            ...
                            api.logger.warn(`[cli-bridge] proxy unavailable after retry: ${e2.message}`);
                        }
                    }
                    else {
                ...
                if (proxyServer) {
                    ...
                    await new Promise((resolve) => {
                        proxyServer.close((err) => {
                            if (err)
                                api.logger.warn(`[cli-bridge] proxy close error: ${err.message}`);
                            else
                                api.logger.info(`[cli-bridge] proxy server closed on plugin stop`);
                            resolve();
                        });
                    });
                    proxyServer = null;
        ...
        api.registerCommand({
            ...
            handler: async (ctx) => {
                ...
                try {
                    const result = await api.runtime.system.runCommandWithTimeout(["openclaw", "models", "set", prev], { timeoutMs: 8_000 });
                    if (result.code !== 0) {
                        const err = (result.stderr || result.stdout || "unknown error").trim();
        ...
        api.registerCommand({
            ...
            handler: async (ctx) => {
                ...
                if (!pending) {
                    const current = readCurrentModel();
                    return {
                        text: `ℹ️ No staged switch pending.\n` +
        ...
        api.registerCommand({
            ...
            handler: async () => {
                ...
                if (!pending) {
                    return {
                        text: `✅ No pending switch.\n` +
                            `Current model: \`${current ?? "unknown"}\``,
                    };
                }
                return {
                    text: `📋 **Staged switch pending:**\n` +
        ...
        api.registerCommand({
            ...
            handler: async () => {
                ...
                for (const c of CLI_MODEL_COMMANDS) {
                    ...
                    if (c.model.startsWith("vllm/cli-claude/"))
                        groups["Claude Code CLI"].push(entry);
                    else if (c.model.startsWith("vllm/cli-gemini/"))
        ...
        api.registerCommand({
            ...
            requireAuth: false,
            handler: async () => {
                const lines = [`🌉 *CLI Bridge Help* — v${plugin.version}`, ""];
                ...
                for (const c of CLI_MODEL_COMMANDS) {
                    if (c.model.startsWith("vllm/cli-claude/"))
                        cliGroups["Claude Code"].push(c);
                    else if (c.model.startsWith("vllm/cli-gemini/"))
                        cliGroups["Gemini"].push(c);
                }
        ...
        api.registerCommand({
            ...
            handler: async () => {
                ...
                const ctx = await getOrLaunchGrokContext((msg) => api.logger.info(msg));
                if (!ctx)
                    return { text: "❌ Could not launch browser. Check server logs." };
                if (importedCookies.length > 0) {
                    await ctx.addCookies(importedCookies);
            ...
            handler: async () => {
                if (geminiContext) {
                    ...
                    try {
                        ...
                        const editor = await page.locator(".ql-editor").isVisible().catch(() => false);
                        if (editor)
                            return { text: "✅ Already connected to gemini.google.com. Use `/gemini-logout` first to reset." };
                    }
                    catch { /* fall through */ }
                    geminiContext = null;
                }
                ...
                try {
                    ...
                    return { text: `❌ Failed to open gemini.google.com: ${err.message}` };
                }
            ...
            handler: async () => {
                if (claudeWebContext) {
                    ...
                    try {
                        ...
                        const editor = await page.locator(".ProseMirror").isVisible().catch(() => false);
                        if (editor)
                            return { text: "✅ Already connected to claude.ai. Use `/claude-logout` first to reset." };
                    }
                    catch { /* fall through */ }
                    claudeWebContext = null;
                }
                ...
                try {
                    ...
                    return { text: `❌ Failed to open claude.ai: ${err.message}` };
                }
        ...
        api.registerCommand({
            ...
            handler: async () => {
                ...
                try {
                    ...
                    return { text: `❌ Failed to open chatgpt.com: ${err.message}` };
                }
        ...
        api.registerCommand({
            ...
            handler: async () => {
                const lines = [`🌉 *CLI Bridge v${plugin.version} — Provider Status*\n`];
                const checks = [
                    {
                        name: "Grok",
                        ctx: grokContext,
