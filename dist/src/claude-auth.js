 ...
 */
export async function readTokenExpiry() {
    try {
...
export async function ensureClaudeToken() {
    const expiresAt = await readTokenExpiry();
    if (expiresAt === null)
        return; // API key user, nothing to do
    const msUntilExpiry = expiresAt - Date.now();
    if (msUntilExpiry > REFRESH_SYNC_WINDOW_MS)
...
async function doRefresh() {
    ...
    if (result.exitCode !== 0) {
        ...
        if (stderr.includes("401") || stderr.includes("authentication_error") || stderr.includes("Invalid authentication credentials")) {
            throw new Error("Claude CLI OAuth token refresh failed (401). " +
...
function runRefreshPing() {
    return new Promise((resolve) => {
        ...
        const proc = spawn("claude", ["-p", "ping", "--output-format", "text", "--permission-mode", "plan", "--tools", ""], { timeout: REFRESH_TIMEOUT_MS, env });
        let stdout = "";
        let stderr = "";
