...
export const DEFAULT_SESSION_PATH = join(homedir(), ".openclaw", "grok-session.json");
...
export function isSessionExpiredByAge(session) {
 ...
 */
export async function verifySession(context, log) {
...
// ──────────────────────────────────────────────────────────────────────────────
export async function runInteractiveLogin(browser, sessionPath, log, timeoutMs = 5 * 60 * 1000) {
    ...
    await page.waitForSelector('textarea, [placeholder*="mind"], [aria-label*="message"]', { timeout: timeoutMs });
...
// ──────────────────────────────────────────────────────────────────────────────
export async function createContextFromSession(browser, session) {
    const context = await browser.newContext({
...
//# sourceMappingURL=grok-session.js.map