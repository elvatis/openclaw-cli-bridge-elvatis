 ...
 */
export async function getOrCreateClaudePage(context) {
...
async function sendAndWait(page, message, timeoutMs, log) {
    ...
    while (Date.now() < deadline) {
        ...
        if (currentCount <= countBefore)
            continue; // response not started yet
...
// ─────────────────────────────────────────────────────────────────────────────
export async function claudeComplete(context, opts, log) {
...
export async function claudeCompleteStream(context, opts, onToken, log) {
    const model = resolveModel(opts.model);
    const prompt = flattenMessages(opts.messages);
    const timeoutMs = opts.timeoutMs ?? DEFAULT_TIMEOUT_MS;
...
//# sourceMappingURL=claude-browser.js.map