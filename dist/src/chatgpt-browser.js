...
export async function getOrCreateChatGPTPage(context) {
    ...
    if (existing.length > 0)
        return { page: existing[0], owned: false };
...
async function isStreaming(page) {
    return page.evaluate(() => !!document.querySelector('button[data-testid="stop-button"]'));
}
/**
 * Send a message and wait for stable response.
 */
async function sendAndWait(page, message, timeoutMs, log) {
    ...
    await page.evaluate((msg) => {
        ...
        if (!ed)
            throw new Error("ChatGPT editor (#prompt-textarea) not found");
        ed.focus();
...
// ─────────────────────────────────────────────────────────────────────────────
export async function chatgptComplete(context, opts, log) {
...
export async function chatgptCompleteStream(context, opts, onToken, log) {
    const model = resolveModel(opts.model);
    const prompt = flattenMessages(opts.messages);
    const timeoutMs = opts.timeoutMs ?? DEFAULT_TIMEOUT_MS;
    const navUrl = MODEL_URLS[model] ?? CHATGPT_HOME;
    ...
    try {
        ...
        await page.evaluate((msg) => {
            const ed = document.querySelector("#prompt-textarea");
            if (!ed)
                throw new Error("ChatGPT editor not found");
            ed.focus();
            document.execCommand("insertText", false, msg);
        }, prompt);
        await new Promise((r) => setTimeout(r, 300));
