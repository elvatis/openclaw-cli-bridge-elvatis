...
function statusBadge(p) {
    if (p.ctx !== null)
        return { label: "Connected", color: "#22c55e", dot: "\u{1F7E2}" };
    if (!p.expiry)
        return { label: "Never logged in", color: "#6b7280", dot: "\u26AA" };
    if (p.expiry.startsWith("\u26A0\uFE0F EXPIRED"))
        return { label: "Expired", color: "#ef4444", dot: "\u{1F534}" };
    if (p.expiry.startsWith("\u{1F6A8}"))
        return { label: "Expiring soon", color: "#f59e0b", dot: "\u{1F7E1}" };
...
export function renderActiveRequests(active) {
    if (active.length === 0) {
        return `
    <div class="card">
...
export function renderRecentRequestLog(entries) {
    if (entries.length === 0) {
        return `
    <div class="card">
...
export function renderFallbackHistory(events) {
    if (events.length === 0) {
        return `
    <div class="card">
...
export function renderProviderSessions(sessions) {
...
export function renderTimeoutConfig(config) {
...
function renderModels(models, modelCommands) {
    ...
    const modelList = (items) => items.map(m => {
        const cmd = cmds[m.id];
...
export function renderDashboardData(opts) {
    return {
...
export function renderStatusPage(opts) {
    ...
    const navHtml = NAV_ITEMS.map(n => `<a href="#${n.id}" class="nav-item" data-nav="${n.id}" onclick="showSection('${n.id}')">${n.icon} ${n.label}</a>`).join("\n      ");
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
//# sourceMappingURL=status-template.js.map