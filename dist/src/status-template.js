...
function statusBadge(p) {
    if (p.ctx !== null)
        return { label: "Connected", color: "#22c55e", dot: "🟢" };
    if (!p.expiry)
        return { label: "Never logged in", color: "#6b7280", dot: "⚪" };
    if (p.expiry.startsWith("⚠️ EXPIRED"))
        return { label: "Expired", color: "#ef4444", dot: "🔴" };
    if (p.expiry.startsWith("🚨"))
        return { label: "Expiring soon", color: "#f59e0b", dot: "🟡" };
...
// ── Recent Request Log ────────────────────────────────────────────────────���
function renderRecentRequestLog(entries) {
    if (entries.length === 0) {
        return `
    <div class="card">
...
// ── Fallback History ───────────────────────────────────────────────────────
function renderFallbackHistory(events) {
    if (events.length === 0) {
        return `
    <div class="card">
...
function renderTimeoutConfig(config) {
...
// ── Metrics sections ────────────────────────────────────────────────────────
function renderMetricsSection(m) {
