 ...
 */
export async function getOrCreateGrokPage(context) {
...
async function sendAndWait(page, message, timeoutMs, log) {
    ...
    await page.evaluate((msg) => {
        ...
        if (!ed)
            throw new Error("Grok editor not found");
        ed.focus();
    ...
    while (Date.now() < deadline) {
        ...
        const text = await page.evaluate((before) => {
 ...
 */
export async function grokComplete(context, opts, log) {
 ...
 */
export async function grokCompleteStream(context, opts, onToken, log) {
    const model = resolveModel(opts.model);
    const prompt = flattenMessages(opts.messages);
    const timeoutMs = opts.timeoutMs ?? DEFAULT_TIMEOUT_MS;
