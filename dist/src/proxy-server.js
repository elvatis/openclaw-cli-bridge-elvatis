...
async function handleRequest(req, res, opts) {
    ...
    if (url === "/healthz" && req.method === "GET") {
        ...
        const sessionStatus = (provider, ctxGetter, expiryStr) => {
            const connected = ctxGetter?.() !== null && ctxGetter?.() !== undefined;
            let status = "not_configured";
            if (connected)
                status = "connected";
            else if (expiryStr?.startsWith("⚠️ EXPIRED"))
                status = "expired";
            else if (expiryStr)
                status = "logged_in";
        ...
        if (model.startsWith("web-grok/")) {
            ...
            try {
                if (stream) {
                    ...
                    const result = await doGrokCompleteStream(grokCtx, { messages: grokMessages, model: grokModel, timeoutMs }, (token) => sendSseChunk(res, { id, created, model, delta: { content: token }, finish_reason: null }), opts.log);
...
// Helpers
// ──────────────────────────────────────────────────────────────────────────────
function sendSseChunk(res, params) {
    const chunk = {
        id: params.id,
