...
const DEFAULT_AUTH_STORE_PATH = join(homedir(), ".openclaw", "agents", "main", "agent", "auth-profiles.json");
/**
...
export async function importCodexAuth(opts) {
    ...
    try {
        ...
        const msg = `Codex auth not available: ${err.message}`;
        log(msg);
    ...
    try {
        if (existsSync(authStorePath)) {
            store = JSON.parse(readFileSync(authStorePath, "utf8"));
        }
        else {
        ...
        const msg = `Cannot read auth store at ${authStorePath}: ${err.message}`;
        log(msg);
        return { imported: false, skipped: false, error: msg };
    }
    ...
    try {
        ...
        const msg = `Failed to write auth store: ${err.message}`;
        log(msg);
        return { imported: false, skipped: false, error: msg };
    }
}
//# sourceMappingURL=codex-auth-import.js.map