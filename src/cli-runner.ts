...
export async function routeToCliRunner(
  ...
  if (resolved.startsWith("cli-gemini/"))        rawText = await runGemini(prompt, resolved, timeoutMs, opts.workdir, { tools: opts.tools, log });
  ...
  else if (resolved.startsWith("cli-grok/"))     rawText = await runGrok(prompt, resolved, timeoutMs, opts.workdir, { tools: opts.tools, log });
    ...
    `Unknown CLI bridge model: "${model}". Use "vllm/cli-gemini/<model>", "vllm/cli-claude/<model>", "vllm/cli-grok/<model>", "openai-codex/<model>", "opencode/<model>", or "pi/<model>".`
  );
