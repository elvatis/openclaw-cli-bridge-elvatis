...
async function handleRequest(
  ...
  if (url === "/v1/chat/completions" && req.method === "POST") {
    ...
    if (perplexityApiModel.startsWith("perplexity-api/")) {
      ...
      try {
        if (stream) {
          ...
          const result = await perplexityApiCompleteStream(
            cleanMessages,
            apiOpts,
            (token) => sendSseChunk(res, { id, created, model: perplexityApiModel, delta: { content: token }, finish_reason: null })
          );
          const estComp = estimateTokens(result.content);
    ...
    if (openrouterApiModel.startsWith("openrouter-api/")) {
      ...
      try {
        if (stream) {
          ...
          const result = await openrouterApiCompleteStream(
            cleanMessages,
            apiOpts,
            (token) => sendSseChunk(res, { id, created, model: openrouterApiModel, delta: { content: token }, finish_reason: null })
          );
          const estComp = estimateTokens(result.content);
      ...
      if (stream) {
        ...
        try {
          ...
          const delegatePrompt = (typeof lastUser?.content === "string" ? lastUser!.content
            : Array.isArray(lastUser?.content) ? (lastUser!.content as Array<{ type: string; text?: string }>).filter(p => p.type === "text").map(p => p.text as string).join(" ")
            : userText.slice(-2000)) as string;
    ...
    let result: CliToolResult = { content: "" };
    let usedModel = model;
    ...
    try {
      ...
      if (fallbackChain.length > 0) {
        ...
        const inToolLoop = hasTools && (lastMsg?.role === "tool" || (lastMsg?.role as string) === "function");
